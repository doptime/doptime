...
func LoadConfig_FromFile() {
	...
	if _, err := toml.DecodeFile(tomlFile, &Cfg); err == nil {
		log.Info().Str("filename", tomlFile).Msg("LoadConfigFromFile success")
		return
	}

	//if toml file exist, but with bad format, then return
	if _, err := os.Stat(tomlFile); os.IsExist(err) {
		log.Error().Err(err).Str("filename", tomlFile).Msg("LoadConfigFromFile failed, see example format in config.demo.toml")
	}
	...
	Cfg.Redis = append(Cfg.Redis, ConfigRedis{Name: "redis_server_demo", Username: "goflow", Password: "yourpasswordhere", Host: "drangonflydb.local", Port: 6379, DB: 0})

	//open file to write
	if writer, err = os.OpenFile(demoConfigFile, os.O_CREATE|os.O_WRONLY, 0644); err != nil {
		log.Error().Err(err).Str("filename", demoConfigFile).Msg("save demo toml config file failed")
		return
	}
	defer writer.Close()
	//encode toml
	...
	if err := encoder.Encode(Cfg); err != nil {
		log.Error().Err(err).Str("filename", demoConfigFile).Msg("toml Encode demo toml config file failed")
	}
}
